...
WebMIDIAPIWrapper.prototype = {
    ...
    parseMIDIMessage: function(msg) {
        ...
        for(var i=0; i<msg.data.length; i++) {
            msg16.push(msg.data[i].toString(16));
        }
        ...
        if((msg[0] & 0xf0) == 0xf0) {
            ...
            switch(event.statusNum) {
              case "8":
                ...
                event.noteNumber=msg.data[1];
                event.velocity=msg.data[2];
                break;
              case "9":
              ...
              case "a":
                ...
                event.noteNumber=msg.data[1];
                event.amount=msg.data[2];
                break;
              case "b":
                event.subType="controller";
		event.ctrlNo = msg.data[1];
		event.value = msg.data[2];
                switch(event.ctrlNo) {
                  case 0x00:
                  case "0x00":
		...
		event.amount = msg.data[1];
                break;
				      case "e":
					      event.subType = 'pitchBend';
                var msb=msg.data[2], lsb=msg.data[1];
